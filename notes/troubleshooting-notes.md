# Troubleshooting Notes — Lab 54 Attacker Cleanup Investigation
...
|---|---|
| PowerShell script blocked | Used process-level `-ExecutionPolicy Bypass` |
| Security 4688 unavailable | Used Sysmon 1 and PowerShell 4104 |
| 4104 observed before cleanup | Used as script execution evidence |
| 4104 not observed after cleanup | Documented as telemetry gap |
| Sysmon 1 available | Used for process investigation |
| Sysmon 3 checked | Used for network context |
| Test files deleted | Compared against pre-cleanup baseline |
| Baseline metadata available | Preserved pre-cleanup evidence |
| Baseline hashes available | Preserved artifact identifiers |
| Cleanup needed to be simulated | Restricted deletion to lab directory |
...
The most valuable evidence in a cleanup investigation may be information collected before the attacker removes files.

## 2. Missing Events Are Not Proof of Absence

If 4104 or 4688 is missing, document the limitation rather than assuming the activity never occurred.

## 3. Do Not Permanently Weaken Security Controls

Use the smallest possible configuration change for a controlled lab.

The process-level PowerShell bypass was used instead of changing the system-wide Execution Policy.

## 4. Separate Cleanup From Evidence Destruction

A safe DFIR lab should simulate deletion only inside a dedicated test directory.

Real Windows logs and security infrastructure should remain untouched.

## 5. Correlate Multiple Evidence Sources

The strongest reconstruction uses:
...
The central troubleshooting lesson from Lab 54 is that attacker cleanup can remove visible filesystem artifacts without necessarily removing every trace of execution.

A strong DFIR investigation therefore collects evidence early, preserves logs, records baseline file state, and correlates the remaining telemetry after cleanup.

The goal is not simply to find the deleted file.

The goal is to reconstruct the activity that occurred before the file disappeared.
